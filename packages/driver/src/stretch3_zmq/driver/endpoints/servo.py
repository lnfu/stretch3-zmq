"""Servo endpoint: end-effector Cartesian control via SUB socket."""
...
def servo_endpoint(config: DriverConfig, robot: StretchRobot) -> NoReturn:
    """
    Servo endpoint: receive relative EE pose + absolute gripper, execute via IK.
    ...
    with zmq_socket(zmq.SUB, f"tcp://*:{config.ports.servo}") as socket:
        ...
        logger.info(f"Servo endpoint started. Listening on tcp://*:{config.ports.servo}")
