...
from stretch3_zmq.driver.endpoints.zmq_helpers import zmq_socket, zmq_socket_pair  # noqa: E402
...
class TestZmqSocket:
    ...
    def test_pub_socket_sndhwm_set_to_8(self) -> None:
        with zmq_socket(zmq.PUB, f"tcp://127.0.0.1:{_BASE_PORT + 2}") as socket:
            assert socket.getsockopt(zmq.SNDHWM) == 8

    def test_sub_socket_rcvhwm_set_to_8(self) -> None:
        with zmq_socket(zmq.SUB, f"tcp://127.0.0.1:{_BASE_PORT + 3}") as socket:
            assert socket.getsockopt(zmq.RCVHWM) == 8

    def test_non_pub_sub_socket_has_no_hwm_override(self) -> None:
        """PUSH socket should use ZMQ's default HWM (not forced to 8)."""
...
class TestZmqSocketPair:
    ...
    def test_both_sockets_have_sndhwm_8(self) -> None:
        with zmq_socket_pair(
            f"tcp://127.0.0.1:{_BASE_PORT + 12}",
            f"tcp://127.0.0.1:{_BASE_PORT + 13}",
        ) as (a, b):
            assert a.getsockopt(zmq.SNDHWM) == 8
            assert b.getsockopt(zmq.SNDHWM) == 8
